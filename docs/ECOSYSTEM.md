# PitCrew Mechanics and PitCrew

**Two surfaces of one product.** Not two products, and not a migration.

> **On the two names.** The demand engine was called *HostPitCrew* until
> 2026-09-21, when it was renamed *PitCrew Mechanics* and its apex moved to
> `pitcrewmechanics.com`. This repo is *PitCrew* — the transaction. The names
> are now close enough to confuse, so throughout this repo: **PitCrew
> Mechanics** (`mauricegoring/PitCrewMechanics`) is the demand engine, and
> **PitCrew** unqualified is this repo. See
> `PitCrewMechanics/docs/domain-rename-2026-09-21.md`.
>
> **The lowercase `hostpitcrew` identifiers did not move and must not.** They
> are wire values and stored data, not names — renaming one breaks the seam or
> orphans a row. They are listed under "Identifiers that keep the old spelling"
> below. A future rename pass that sweeps them together breaks verification
> hand-off for every host who already used it.

| | PitCrew Mechanics | PitCrew |
|---|---|---|
...
PitCrew Mechanics is not deprecated and is not a legacy system. It is the half of a
...
founder's time; demand is not. PitCrew Mechanics already ranks, already captures, and
...
PitCrew Mechanics already proved the demand is there. That is the entire reason to
keep both running.
...
A host who verified on PitCrew Mechanics — by Turo CSV or by FK Command Center link —
...
three ways to earn it, one place to ask whether it is held. The method value
keeps its original spelling — see below.

### 2. Demand, in aggregate, and only in aggregate

PitCrew Mechanics exports counts: metro × service × lane × period → searches, leads.
...
Individual host data never leaves PitCrew Mechanics's admin surface. That is a
...
A thousand PitCrew Mechanics searches say a metro wants mechanics. They say nothing
...
1. PitCrew Mechanics demand crosses the bar (`metro_launch_signal`): searches, leads,
...
3. PitCrew opens in that metro. The demand multiplier sits at 1.000 until real
   bookings accumulate.
4. PitCrew Mechanics starts routing that metro's bookable demand to PitCrew. Metros
...
## Identifiers that keep the old spelling

The product name moved; these did not. Each is a value that is either already
on the wire between the two systems or already written into rows in the
database. Renaming one does not rename a product — it breaks a join, rejects a
valid claim, or silently stops matching existing data.

| Identifier | Where | What breaks if renamed |
| --- | --- | --- |
| `ISSUER = 'hostpitcrew'` | `lib/handoff.js` | Every claim PitCrew Mechanics mints carries `iss: 'hostpitcrew'`. Change one side only and every hand-off is rejected; change both and claims already in flight are rejected. |
| `method IN (…,'hostpitcrew')` | `db/007-ecosystem.sql` | The stored verification method on existing `host_verifications` rows. A renamed CHECK rejects the rows already written under the old value. |
| `source DEFAULT 'hostpitcrew'` | `db/007-ecosystem.sql` | The provenance stamp on every `imported_demand` row already loaded. |
| `hpc:host:*` subject prefix | `test/schema.test.js` | The opaque subject format PitCrew Mechanics mints. It is the other side's namespace, not ours to rename. |

The same rule, and the same reasoning, as the identifier table in
`PitCrewMechanics/docs/domain-rename-2026-09-21.md`. If this repo ever grows a
`lib/brand.js` the way PitCrew Mechanics did, these values still do not belong
in it: a brand file holds names, and none of these are names.
